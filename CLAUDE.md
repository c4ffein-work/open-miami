...
## Music
- Two modules: `src/music.rs` (host-compiled + unit-tested: the song FORMAT, the theory helpers that read it, every song) and `src/audio.rs` (wasm-only: the Web Audio synthesizer + SFX; NO audio files, no crates). Authoring reference: `docs/MUSIC_FORMAT.md`. Songs are `const` data: `SongSpec { name, root (Hz), scale (mode), bpm, steps_per_beat, voices: [Voice; NUM_VOICES] (bass/lead/pad/arp/keys), sections, intensity, swing (0..1 triplet shuffle of the odd 16ths), humanize (s, ≤ 0.02, every note but the kicks), sweep (0..1 depth of the per-bar bus wah), sidechain: Sidechain { depth, release_beats } (kick-driven ducker, `Sidechain::OFF`), echo: Echo { steps, feedback, tone } (`Echo::DOTTED`) }`; a `Section` (author with `..Section::EMPTY`) is seven step lanes — five melodic (`bass lead pad arp keys`: scale degrees) + two percussion (`drums perc`: `Drum::{Silent, Kick, Hat, Snare, Clap, OpenHat, Tom, Rim, Crash}`) — plus optional velocity lanes (`*_vel`: `0..=MAX_VEL` (9) per step, empty = full, 0 = skip) and chord lanes (`*_chord`: `Chord::{Single, Octave, Power, Triad, Sus2, Sus4, Seventh, Add9, Inv1, Inv2, Open}` = scale-degree offsets, empty = Triad for the pad / Single otherwise; partials at 1/√n level). Lanes loop inside a section (length = the longest note/drum lane). Sentinels: `REST`, `HOLD` = TIE (length = 1 + trailing `HOLD`s, wrapping; `note_at` → `NoteOn { degree, len, from }` where `from` = the touching previous note = LEGATO). Channel indices `BASS LEAD PAD ARP KEYS DRUMS PERC`, `MELODIC`, `NUM_CHANNELS` = 7; `SONGS` (index = `play_song`), `song_for_floor(level)`; tracker cells `cell_at → Cell::{Off, On(vel), Hold}`, info helpers `scale_name` / `note_name` / `voice_summary`
- `Voice` = one melodic lane's instrument, `const fn` builders: `Voice::mono / panned(wave, pan) / wide(wave, pan, detune, width) / stack(wave, pan, detune, width, unison ≤ 7)` then `.with_filter(cutoff, peak, attack, decay, q)` (per-note LOWPASS envelope: attack > 0 = bloom, decay > 0 = wow), `.with_vibrato(rate, cents, delay)`, `.with_env(attack, gate_steps)` (overrides the lane pluck: bass 1.9 / lead 0.9 / pad 4.0 / arp 0.7 / keys 1.2 steps), `.with_drive(0..1)` (lane-level tanh shaper), `.with_echo(send)`, `.with_reverb(send)`, `.with_sub(level)` (sine an octave down), `.with_glide(s)` (portamento on legato notes); `Wave::{Sine, Triangle, Square, Sawtooth, Noise}` (`Noise` = the noise buffer under the envelope + filter: risers)
- Engine (`AudioEngine`): look-ahead scheduler (`LOOKAHEAD` 0.15 s on the audio clock; swing/humanize applied per note). Per melodic lane: `StereoPannerNode` → `WaveShaperNode` (drive, `None` curve = bypass) → echo/hall send gains (`MusicFx`: one tempo-synced `DelayNode` with feedback through a lowpass, one `ConvolverNode` with a synthesized 2.6 s hall) → the side-chain ducker `GainNode` (kick: computed current level → `1 − depth` in 4 ms → `set_target_at_time` release; `duck_level` is the pure curve) → music bus → lowpass swept once per bar (peak from intensity, floor from `sweep`) → limiter → out; drums straight into the bus. Envelope per note (`tone_env(Tone)`): attack → peak, HELD for the tied steps, then the pluck; glide = exponential pitch ramp over `Voice::glide`; unison stack per partial (0.78/√n each, side panners for width, a per-oscillator biquad for the filter envelope, an LFO for vibrato); `Voice::sub` = one extra sine. Every distinct `MusicKey` (`Note { lane, degree, len, chord, from }` | `Drum(Drum)`, enumerated by `music::music_keys` / `note_key`, ≤ 96 per song — tested) is PRE-BAKED through an `OfflineAudioContext` at exact pitch (stereo buffer for a wide voice) and fired as one `AudioBufferSourceNode`; velocity is a play-time `GainNode` only below `MAX_VEL`; `from` enters the key only on gliding voices. Unbaked notes fall back to a LIGHT live sketch (`AudioEngine::sketch`: one plain oscillator per partial, no stack / filter / vibrato / sub — bounded cost); the bake queue is combat SFX → current song → rare SFX. `apply_voices` re-points the persistent lane nodes (pan, drive curve, sends, echo time) on every song change
