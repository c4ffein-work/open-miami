...
## Music
- Two modules: `src/music.rs` (host-compiled + unit-tested: the song FORMAT, the theory helpers that read it, every song) and `src/audio.rs` (wasm-only: the Web Audio synthesizer + SFX; NO audio files, no crates). Songs are `const` data: `SongSpec { name, root (Hz), scale (mode), bpm, steps_per_beat, voices: [Voice; 4] (bass/lead/pad/arp), sections, intensity }`; a `Section` is five step lanes (`bass`/`lead`/`pad`/`arp`: scale degrees, `drums`: `Drum`s) + five optional velocity lanes (`*_vel`: one `0..=MAX_VEL` (9) per step, empty = all full) — author with `..Section::EMPTY`. Lanes loop inside a section (its length = the longest NOTE lane). Sentinels: `REST` = silence, `HOLD` = TIE the previous note through the step (length = 1 + trailing `HOLD`s, wrapping around the lane, `note_at`). Velocity is linear amplitude (`vel_at`); `bass: &[0,0,0,0]` under `bass_vel: &[3,6,8,9]` is the side-chain pump. `Voice { wave: Wave, pan (−1..1), detune (cents: 0 = one oscillator, > 0 = a unison pair at ±detune), width (0..1 stereo spread of the pair) }` via `Voice::mono / panned / wide`. Lane indices `BASS LEAD PAD ARP DRUMS`; `SONGS` (index = `play_song`), `song_for_floor(level)`. Tracker cells = `music::cell_at → Cell::{Off, On(vel), Hold}`
- Engine (`AudioEngine`): look-ahead scheduler (`LOOKAHEAD` 0.15 s on the audio clock), one shared music bus → lowpass swept once per bar → destination; each melodic lane owns a `StereoPannerNode` into the bus at its voice's `pan` (drums straight in). Envelope per note: attack → peak, HELD for the tied steps, then the lane's pluck (`lane_shape`: gate/level/attack per lane; `tone_env`). Every distinct `MusicKey` (`Note { lane, degree, len }` | `Kick` | `Hat` | `Snare`, enumerated by `music::music_keys`, ≤ 96 per song — tested) is PRE-BAKED through an `OfflineAudioContext` at exact pitch (stereo buffer for a wide voice, mono otherwise) and fired as one `AudioBufferSourceNode`; velocity is a play-time `GainNode` only on notes below `MAX_VEL` (never part of the bake set). Unbaked notes fall back to live synthesis; the bake queue is combat SFX → current song → rare SFX
